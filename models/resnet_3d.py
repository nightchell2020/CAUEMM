...
class ResNet3D(nn.Module):
    # MedicalNet version of 3DResNet
    def __init__(
            self,
            block: str,
        ...
        if stride != 1 or self.current_channels != channels * block.expansion:
            if shortcut_type == 'A':
                downsample = partial(self._downsample_basic_block,
                                     planes=channels * block.expansion,
                                     stride=stride)
            else:
                downsample = nn.Sequential(
                    conv1x1x1(self.current_channels, channels * block.expansion, stride),
    ...
    def compute_feature_embedding(self, x, target_from_last: int = 0):
        ...
        x = checkpoint(self.conv_stage4,x,use_reentrant=False)
